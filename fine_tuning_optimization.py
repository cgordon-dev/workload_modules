...
"""Fine-tuning optimization workload for StyleGAN2-ADA with easy dataset swapping"""
...
def setup_fine_tuning_config(config):
    """Set up fine-tuning configuration based on parsed arguments or config file.
    
    Args:
        config: Configuration dictionary with fine-tuning parameters
        
    Returns:
        args: Training arguments for StyleGAN2-ADA fine-tuning
    """
    args = dnnlib.EasyDict()
    
    # Ensure resume parameter is set for fine-tuning
    if 'resume' not in config or not config['resume']:
        raise UserError('Fine-tuning requires a pretrained model specified via --resume')
    ...
    # Fine-tuning specific settings - shorter training duration
    if config.get('kimg') is None:
        config['kimg'] = 1000  # Default to shorter training for fine-tuning
    ...
    # Base network config - use auto by default for fine-tuning
    ...
    if cfg == 'auto':
        ...
        spec.lrate = 0.0008 if res >= 1024 else 0.001  # Lower learning rate for fine-tuning
    ...
    # Fine-tuning specific learning rate override
    if config.get('lr') is not None:
        # For fine-tuning, allow direct override of learning rate
        spec.lrate = config.get('lr')
    ...
    if aug == 'ada':
        ...
        if config.get('p') is None:
            args.augment_p = 0.5  # Use a reasonable default for fine-tuning
        else:
    ...
    # Transfer learning - use the provided model for fine-tuning
    resume_specs = {
    ...
    # Freezing layers for transfer learning - default to some frozen layers
    freezed = config.get('freezed', 4)  # Default to 4 layers frozen for fine-tuning
    if freezed > 0:
...
@click.command()
@click.option('--dataset-path', type=str, help='Path to the fine-tuning dataset directory or zip file', required=True)
@click.option('--resume', type=str, help='Pretrained model to fine-tune (path, URL, or preset name)', required=True)
...
@click.option('--kimg', type=int, help='Override fine-tuning duration in thousands of images', default=1000)
@click.option('--freezed', type=int, help='Number of layers to freeze in discriminator', default=4)
@click.option('--lr', type=float, help='Learning rate override', default=None)
@click.option('--use-labels', type=bool, help='Use labels from dataset.json', default=True)
@click.option('--dry-run', is_flag=True, help='Print options and exit')
@click.option('--aug', type=click.Choice(['noaug', 'ada', 'fixed']), help='Augmentation mode', default='ada')
@click.option('--p', type=float, help='Augmentation probability for --aug=fixed')
@click.option('--industry', type=str, help='Industry name for organization', default=None)
def main(**kwargs):
    """Fine-tune a StyleGAN2-ADA model with customizable dataset.
    
    Examples:
    
    # Fine-tune an automotive model at 256x256 resolution
    python fine_tuning_optimization.py --dataset-path=./datasets/automotive --image-size=256 --outdir=./results --resume=ffhq256 --kimg=1000
    
    # Fine-tune a medical model with frozen layers using a config file
    python fine_tuning_optimization.py --dataset-path=./datasets/medical --config=./configs/medical_finetuning.yaml --outdir=./results
    
    # Fine-tune a fashion model with a custom learning rate
    python fine_tuning_optimization.py --dataset-path=./datasets/fashion --resume=./results/network-snapshot-000100.pkl --outdir=./results --lr=0.0005
    """
    ...
    industry_tag = f"-{config.get('industry')}" if config.get('industry') else ""
    desc = f"stylegan2-ada-finetuned{industry_tag}-{Path(args.training_set_kwargs.path).stem}"
    ...
    print(f'Pretrained model: {args.resume_pkl}')
    print(f'Fine-tuning duration: {args.total_kimg} kimg')
    print(f'Frozen layers: {args.D_kwargs.block_kwargs.freeze_layers}')
    ...
    with open(os.path.join(args.run_dir, 'fine_tuning_options.json'), 'wt') as f:
        json.dump(args, f, indent=2)
    
    # Launch training process
    if args.num_gpus == 1:
        subprocess_fn(0, args, None)
    else:
